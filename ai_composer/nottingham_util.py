...
def prepare_nottingham_pickle(
    time_step, chord_cutoff=64, filename=PICKLE_LOC, verbose=False, path="data/Nottingham/"
):
    """
    ...
    for d in ["train", "test", "valid"]:
        ...
        parsed = parse_nottingham_directory(
            "{}{}".format(path, d), time_step, verbose=False
        )
    ...
    def combine(melody, harmony):
        ...
        # for all melody sequences that don't have any notes,
        # add the empty melody marker (last one)
        ...
        for h in harmony:
            if h in chord_mapping:
                harmony_idxs.append(chord_mapping[h])
            elif NO_CHORD in chord_mapping:
                harmony_idxs.append(chord_mapping[NO_CHORD])
            else:
                harmony_idxs.append(0)                
...
def parse_nottingham_to_sequence(input_filename, time_step, verbose=False):
    ...
    melody_sequence = midi_util.round_notes(
        melody_notes,
        track_ticks,
        time_step,
        ...
        if len(notes) > 0:
            ...
            if len(chord) == 0:
                if verbose:
                    print (
                        """
                        Could not determine chord: {} ({}, {}),
                        defaulting to last steps chord
                        """.format(
                            notes_shift, input_filename, i
                        )
                    )
                if len(harmonies) > 0:
...
class NottinghamSampler(object):
    def __init__(
        ...
        chord_to_idx,
        method="sample",
        harmony_repeat_max=16,
        melody_repeat_max=16,
        verbose=False,
    ):
    ...
    def visualize_probs(self, probs):
        ...
        melodies = sorted(
            list(enumerate(probs[:NOTTINGHAM_MELODY_RANGE])),
            key=lambda x: x[1],
            reverse=True,
        )[:4]
        harmonies = sorted(
            ...
            for ts_targets, ts_probs in zip(batch_targets, batch_probs[num_time_steps]):
                ...
                for seq_idx in range(ts_targets.shape[1]):
                    for step_idx in range(ts_targets.shape[0]):
                        idxed = [
                            (n, p) for n, p in enumerate(ts_probs[step_idx, seq_idx, :])
                        ]
                        notes = [n[0] for n in idxed]
                        ps = np.array([n[1] for n in idxed])
                        r = NOTTINGHAM_MELODY_RANGE
...
def create_model(filename=PICKLE_LOC, path="data/Nottingham/"):
    resolution = 480
    time_step = 120
    ...
    prepare_nottingham_pickle(time_step, filename=filename, path=path, verbose=True)
    print ("Model created!")
